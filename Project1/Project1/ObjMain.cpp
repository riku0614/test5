	...
	if (room_in == false)
	{
		for (int i = 0; i < MAP_X; i++)
		{
			for (int j = 0; j < MAP_Y; j++)
			{
				...
				if (m_map[i][j] != 21 && m_map[i][j] <= 99 && m_map[i][j] > 4 && m_map[i][j] != 7 || m_map[i][j] == 3 && m_map[i][j] != 26)
				{
					//要素番号を座標に変更
					float bx = j * 64.0f;
					float by = i * 64.0f;
	...
	else
	{
		for (int i = 0; i < ROOM_X; i++)
		{
			for (int j = 0; j < ROOM_Y; j++)
			{
				...
				if (r_map[i][j] <= 99 && r_map[i][j] > 4 && r_map[i][j] != 7 && r_map[i][j] != 26 || r_map[i][j] == 3)
				{
					//要素番号を座標に変更
					float bx = j * 64.0f;
					float by = i * 64.0f;
		...
		for (int i = 0; i < MAP_X; i++)
		{
			for (int j = 0; j < MAP_Y; j++)
			{
				if (m_map[i][j] > 0)
				{
					...
					//床テクスチャ
					if (m_map[i][j] == 1 || m_map[i][j] == 4 || m_map[i][j] == 7 || m_map[i][j] == 13 || m_map[i][j] == 21 || m_map[i][j] == 26)
					{
						src.m_top = 0.0f;
						src.m_left = 0.0f;
